...
4. **Platform: second dialect (mssql hosted default)** — in progress:
   ...
   - **4b** Grow multi-dialect registry toward baseline parity — **in progress** (v1–6; browser/memory remain for later / m8)
   - **4c** Dialect-safe SQL + `run*Async` — **in progress** (`platformNow` / `coalescePlatformNow` across core repos)
