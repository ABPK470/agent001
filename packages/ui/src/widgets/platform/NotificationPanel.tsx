...
export function NotificationPanel() {
  ...
  return (
    <div className="relative z-[1]" ref={panelRef}>
      <button
        className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-transparent text-text-muted transition-colors hover:bg-[var(--hover-fill)] hover:text-text"
        onClick={() => setOpen((v) => !v)}
        title="Notifications"
      >
        <Bell size={15} />
