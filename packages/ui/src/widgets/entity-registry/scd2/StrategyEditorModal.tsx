...
export function StrategyEditorModal({
  ...
  const identityHint = IDENTITY_OPTIONS.find((option) => option.value === identityHandling)?.hint
  ...
  const identityOptions: ListboxOption<EntityRegistryStrategy["identityHandling"]>[] = IDENTITY_OPTIONS.map((option) => ({
    value: option.value,
    label: option.label,
  }))
        ...
        {tab === "form" ? (
          <AdminModalCanvas>
            <form autoComplete="off" onSubmit={(e) => e.preventDefault()} className="contents">
              ...
              <FormSectionCard title="Start from preset" description="Optional — fills the policy fields below. Edit freely after applying.">
              ...
              <FormSectionCard title="Diff policy" description="Columns listed here are excluded from row-hash comparison and not copied from source on update.">
              ...
              <FormSectionCard title="Stamp expressions" description="Target-dialect SQL evaluated at MERGE time. Keys are column names; values are expressions (not source bindings).">
              ...
              <FormSectionCard title="Primary key on merge" description="How the target identity / PK column is handled during MERGE insert.">
                <FormFieldGroup label="PK handling" hint={identityHint}>
                  <Listbox
                    value={identityHandling}
                    onChange={setIdentityHandling}
                    options={identityOptions}
                    size="sm"
                    className="w-full"
                    ariaLabel="Primary key handling during merge"
                  />
                </FormFieldGroup>
              </FormSectionCard>
