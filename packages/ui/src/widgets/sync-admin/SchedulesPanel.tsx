    ...
    >
      ...
      {isAdmin && (
        <div className="mx-5 mt-4 rounded-lg border border-border-subtle bg-panel p-3">
          <div className="grid grid-cols-[1fr_1fr_1.6fr_auto_auto] items-center gap-2 text-xs">
            <input className="input" placeholder="source env" value={draft.source} onChange={(e) => setDraft({ ...draft, source: e.target.value })} />
            <input className="input" placeholder="target env" value={draft.target} onChange={(e) => setDraft({ ...draft, target: e.target.value })} />
            <input className="input font-mono" placeholder="0 */6 * * *  (every 6h)" value={draft.cron} onChange={(e) => setDraft({ ...draft, cron: e.target.value })} />
