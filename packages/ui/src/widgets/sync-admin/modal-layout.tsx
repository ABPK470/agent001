/**
 * Sync-admin modal layout — same visual language as entity-registry Configuration.
 */

import { Search, X } from "lucide-react"
import type { JSX, ReactNode } from "react"

import { FORM_HEADING, HELP_TEXT, META_TEXT, PANEL } from "./design"
export { FormFieldGroup, FormSectionCard } from "../entity-registry/form-section"
...
export function AdminModalRoot({ children }: { children: ReactNode }): JSX.Element {
  return (
    <div className="entity-registry sync-admin relative flex min-h-0 flex-1 flex-col">
      {children}
    </div>
  )
}
...
/** Scrollable body inside a split-pane editor — full width, Configuration-style spacing. */
export function AdminModalCanvas({
  children,
  width = "full",
}: {
  children: ReactNode
  /** full — span the pane (default). Narrow values only for rare compact dialogs. */
...
export function AdminModalRail({ children }: { children: ReactNode }): JSX.Element {
  return (
    <div className="flex min-h-0 flex-col gap-3 overflow-hidden border-b border-border-subtle p-5 lg:border-b-0 lg:border-r">
      {children}
    </div>
  )
}
...
export function AdminRailSection({
  ...
  title: string
  children: ReactNode
  /** Let list sections fill remaining rail height (overrides list). */
  grow?: boolean
}): JSX.Element {
  return (
