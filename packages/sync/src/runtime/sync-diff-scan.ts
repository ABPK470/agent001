...
import { getPublishedSyncDefinitionForHost, type PublishedSyncDefinition } from "../runtime/published-definitions.js"
...
export async function syncDiffScan(input: SyncDiffScanInput): Promise<SyncDiffScanResult> {
  ...
  const definition = getPublishedSyncDefinitionForHost(input.host, asEntityId(input.entityType))
