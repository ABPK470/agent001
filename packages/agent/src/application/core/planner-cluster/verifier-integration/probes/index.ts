  ...
  for (const htmlEntry of htmlArtifacts) {
    ...
    for (const scriptRef of scriptRefs) {
      ...
      if (!scriptRef.isModule) {
        ...
        const usesEsModuleSyntax =
          /\bimport\s+(?:\{|[\w*]|\*\s+as\s+\w)|\bexport\s+(?:default\b|const\b|let\b|var\b|function\b|class\b|\{)/.test(
            resolvedJsContent
          )
...
async function probeCssClassContracts(ctx: IntegrationProbeContext): Promise<void> {
  ...
  const cssContents = await readIntegrationArtifactContents(
    cssArtifacts,
    readFile,
    readArtifactContentViaTool,
    runCommand
  ...
  for (const artifact of codeArtifacts) {
    ...
    const existing = assessments[idx]
    const issues = missingClasses.map(
      (cls) =>
...
async function probeLocalModuleImportBindings(ctx: IntegrationProbeContext): Promise<void> {
  ...
  const fileContents = await readIntegrationArtifactContents(
    codeArtifacts,
    readFile,
    readArtifactContentViaTool,
    runCommand
  ...
  for (const artifact of codeArtifacts) {
    ...
    for (const imported of imports) {
      ...
      if (imported.defaultImport && !exports.hasDefault) {
        issues.push(
          `Import/export mismatch: ${artifact.path} imports default from ${resolved.basename}, but that module has no default export`
        )
      }
      for (const importedName of imported.importedNames) {
        if (!exports.named.has(importedName)) {
          issues.push(
            `Import/export mismatch: ${artifact.path} imports ${importedName} from ${resolved.basename}, but that export is missing`
          )
        }
      }
    }
