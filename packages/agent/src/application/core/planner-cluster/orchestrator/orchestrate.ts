...
import { buildPipelineCallbacks, emitRepairPlanTraces, emitRetryTraces, emitVerificationTraces } from "./traces.js"
...
export async function executePlannerPath(
  ...
  for (let attempt = 0; attempt <= MAX_PIPELINE_RETRIES; attempt++) {
    ...
    const activeCompatibilityPath: "legacy" | "repair" = compatibilityMode === PlannerRepairCompatibilityMode.Repair
      ? "repair"
      : legacyPinnedForRun
        ? "legacy"
        : "repair"
