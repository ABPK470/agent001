...
function recordDoctrineLesson(
  ...
  lesson: { subject: string; claim: string; evidence?: string; category?: string } | null | undefined
): void {
  if (!lesson) return
  try {
...
function buildQueryMssqlTool(host: AgentHost, run?: RunContext): Tool {
  return {
    name: "query_mssql",
    description:
...
function buildSchemaMssqlTool(host: AgentHost, run?: RunContext): Tool {
  return {
    ...
    parameters: {
      ...
      properties: {
        schema: {
          type: "string",
          description:
            "Filter by schema name (e.g., 'agent', 'core', 'publish', 'dim', 'fact'). Lists all tables/views in that schema."
        },
        table: {
          type: "string",
          description:
            "Get detailed column info for a specific table. Use schema prefix for accuracy (e.g., 'agent.PipelineRun', 'core.Dataset', 'fact.AfricaFlex'). Without prefix, searches all schemas."
        },
        search: {
          type: "string",
          description:
            "Search for tables/views by name pattern across all schemas. Uses SQL LIKE with wildcards added automatically. E.g., search='Revenue' finds all tables/views containing 'Revenue' in any schema. search='Pipeline' finds agent.PipelineRun, core.Pipeline, etc."
        },
        connection: {
          type: "string",
          description:
            "Named database connection to use (configured at startup). Omit to use the default connection."
        }
      },
      required: []
    },
        ...
        try {
          ...
          `)
            ...
            if (!result.recordset.length)
              return `No columns found for ${tableName}. Try with schema prefix (e.g. 'agent.${table}', 'core.${table}').`
            ...
            if (schema && surrogateLikeNames.length > 0) {
              ...
              `)
                ...
                const rows = (
                  rangeRes.recordset as Array<{
                    column_name: string
                    min_val: string | null
                    max_val: string | null
                  }>
                ).filter(
                  (r) => wantSet.has(r.column_name.toLowerCase()) && r.min_val !== null && r.max_val !== null
                )
                if (rows.length > 0) {
...
function isIntegerLikeType(dataType: string): boolean {
  ...
  return (
    t === "int" || t === "bigint" || t === "smallint" || t === "tinyint" || t === "numeric" || t === "decimal"
  )
}
