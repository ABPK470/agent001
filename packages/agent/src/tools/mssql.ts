 ...
 *   - Query timeout (60s default, configurable).
...
function validateQuery(query: string, writeEnabled: boolean): string | null {
  ...
  // These queries either time out or run for minutes, regardless of the 60s timeout.
  // The LLM must be told WHY and HOW to fix the query, not just that it failed.
