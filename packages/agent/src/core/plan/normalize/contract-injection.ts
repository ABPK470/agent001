...
import { uniqueList } from "./helpers.js"
...
export function injectSharedDataContract(plan: Plan): void {
  ...
  const jsWriters = subagentSteps.filter((s) =>
    s.executionContext.targetArtifacts.some((a) => /\.js$/i.test(a))
  )
...
export function injectSharedStateOwnershipContract(plan: Plan): void {
  ...
  const contract = {
    ...
    schema:
      "Single shared state object documented by owner file; consumers must read and use that schema without redefining it.",
    mutationPolicy: "owner-only" as const
  }

  for (const step of jsWriters) {
    ;(step.executionContext as unknown as { sharedStateContract?: typeof contract }).sharedStateContract =
      contract
    ...
    if (step.name !== owner.name) {
      ...
      ).requiredSourceArtifacts = [...required]
...
export function injectDependencyWiringCriteria(plan: Plan): void {
  ...
  for (const step of subagentSteps) {
    const hasConsumerArtifact = step.executionContext.targetArtifacts.some((a) =>
      /\.(?:html?|xhtml|xml|md|markdown|svg)$/i.test(a)
    )
    ...
    const hasWiringCriterion = step.acceptanceCriteria.some((c) =>
      /\b(?:load|import|include|link|reference|wire|attach|depends?\s+on|hook(?:s|ed)?\s+up)\b/i.test(c)
    )
...
export function injectBrowserRuntimeContracts(plan: Plan): void {
  ...
  const jsArtifacts = uniqueList(
    jsWriters.flatMap((step) =>
  ...
  for (const step of htmlOwners) {
    ...
    if (
      !step.acceptanceCriteria.some((criterion) =>
        /script tag|type="module"|load.*\.js|runtime artifacts|entry module/i.test(criterion)
      )
    ) {
      ;(step as unknown as { acceptanceCriteria: readonly string[] }).acceptanceCriteria = [
        ...step.acceptanceCriteria,
  ...
  for (const step of jsWriters) {
    ...
    if (
      !step.acceptanceCriteria.includes(
...
export function injectHelperDependencyContracts(plan: Plan): void {
  const codeWriterSteps = plan.steps
  ...
  for (const step of codeWriterSteps) {
    if (
      !/defined in the same file|imported explicitly|dangling references|undefined helper/i.test(
        step.objective
      )
    ) {
      ;(step as { objective: string }).objective =
    ...
    if (
      !step.acceptanceCriteria.some((criterion) =>
        /defined in the same file|imported explicitly|dangling helper|undefined helper|dependency closure/i.test(
          criterion
        )
      )
    ) {
...
export function injectVisualStyleContracts(plan: Plan): void {
  ...
  for (const step of browserSteps) {
    ...
    if (
      !step.acceptanceCriteria.some((criterion) =>
        /css class|visual feedback|row\/column parity|nth-child|interaction state/i.test(criterion)
      )
    ) {
      ;(step as unknown as { acceptanceCriteria: readonly string[] }).acceptanceCriteria = [
        ...step.acceptanceCriteria,
