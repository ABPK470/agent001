...
import { detectPlaceholderPatterns } from "../../governance.js"
...
export async function probeContentCompleteness(
  ...
  const codeArtifacts = sa.executionContext.targetArtifacts.filter((a) =>
...
export function probeCriteriaProof(
  ...
  const runtimeCriterionRe =
    /\b(?:click|submit|drag|drop|keyboard|mouse|interactive|render|display|preview|execute|run|workflow|integration|e2e|end[- ]to[- ]end|api|request|response|endpoint|fetch|http|query|database|sql|persist|sync|auth|login)\b/i
  const complexRuleCriterionRe =
...

  {
    ...
    const blanketComplexClaim =
      /\b(?:all|fully|complete(?:ly)?|properly)\b.{0,80}\b(?:rules|logic|workflow|constraints|requirements)\b/i.test(
        outputText
      )
