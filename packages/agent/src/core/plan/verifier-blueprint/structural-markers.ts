...
export function extractStructureMarkersFromText(text: string): string[] {
  ...
  for (const snippet of snippets) {
    ...
    for (const match of snippet.matchAll(
      /\b([A-Z][A-Za-z0-9]*(?:Panel|View|Component|Layout|Widget|Page|Dialog|Modal|Card|List|Form|Header|Footer|Sidebar|Board|Canvas|Grid))\b/g
    )) {
      markers.push(normalizeStructuralMarker("component", match[1]))
    }
    for (const match of snippet.matchAll(
      /\b(?:function|method|proc(?:edure)?|subroutine|def|fn|lambda|handler|command|cmdlet|label|target)\s+`?([A-Za-z_.$@?-][\w.$@-]*)`?/giu
    )) {
      markers.push(normalizeStructuralMarker("function", match[1]))
    }
    for (const match of snippet.matchAll(
      /\b(?:class|struct|interface|trait|enum|record|module|namespace|package|type)\s+`?([A-Za-z_.$@?-][\w.$@-]*)`?/giu
    )) {
  ...
  for (const match of content.matchAll(/\sid=["']([^"']+)["']/giu)) {
    markers.push(
      ...match[1]
        .split(/\s+/)
        .filter(Boolean)
        .map((value) => normalizeStructuralMarker("id", value))
    )
  }
  for (const match of content.matchAll(/\sclass=["']([^"']+)["']/giu)) {
    markers.push(
      ...match[1]
        .split(/\s+/)
        .filter(Boolean)
        .map((value) => normalizeStructuralMarker("class", value))
    )
  }
...
function extractCodeStructureMarkers(content: string): string[] {
  ...
  markers.push(
    ...collectRegexMarkers(content, "function", /export\s+(?:async\s+)?function\s+([A-Za-z_$][\w$]*)\s*\(/g)
  )
  markers.push(
    ...collectRegexMarkers(content, "function", /(?:^|\n)\s*(?:async\s+)?function\s+([A-Za-z_$][\w$]*)\s*\(/g)
  )
  markers.push(
    ...collectRegexMarkers(content, "function", /export\s+const\s+([A-Za-z_$][\w$]*)\s*=\s*(?:async\s*)?\(/g)
  )
...
export function detectStructuralMarkersInArtifact(path: string, content: string): string[] {
  ...
  if (/\.(?:tsx|jsx)$/i.test(path))
    return uniqueStrings([...extractHtmlStructureMarkers(content), ...extractCodeStructureMarkers(content)])
  if (
    /\.(?:ts|js|mjs|cjs|mts|cts|py|go|rs|java|kt|kts|cs|vb|php|rb|swift|scala|sh|bash|zsh|fish|ps1|psm1|psd1|pl|pm|lua|r|jl|clj|cljs|cljc|lisp|el|asm|s|S|c|cc|cpp|cxx|h|hpp|hh)$/i.test(
      path
    )
  )
    return extractCodeStructureMarkers(content)
  if (
    /\.(?:xml|xaml|csproj|fsproj|vbproj|gradle|properties|toml|yaml|yml|json|ini|cfg|conf|sql|md|txt)$/i.test(
      path
    )
  )
    return uniqueStrings([
      ...extractStructureMarkersFromText(content),
      ...extractCodeStructureMarkers(content)
    ])
  return []
}
