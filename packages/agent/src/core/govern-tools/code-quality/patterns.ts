...
export const PLACEHOLDER_PATTERNS: Array<{ re: RegExp; label: string }> = [
  ...
  {
    re: /\/\/\s*(?:\w+\s+)*(?:to\s+be\s+implemented|to\s+be\s+added|not\s+yet\s+implemented)\b/gi,
    label: "stub comment"
  },
  ...
  {
    re: /\/\/\s*(?:other|rest\s+of(?:\s+the)?|remaining)\s+(?:code|logic|implementation)\b/gi,
    label: "degeneration comment (references code that should be written)"
  },
  {
    re: /\/\/\s*(?:existing|previous|prior)\s+(?:code|logic|implementation)\b/gi,
    label: "degeneration comment (references code that should be written)"
  },
  {
    re: /\/\/\s*(?:same|similar|code continues?)\s+(?:as\s+)?(?:above|before|previously|existing)\b/gi,
    label: "degeneration comment (references code that should be written)"
  },
  {
    re: /\/\/\s*(?:as\s+per|as\s+in)\s+(?:existing|previous|above|the\s+original)\b/gi,
    label: "degeneration comment (references code that should be written)"
  },
  { re: /\/\/\s*\.{3}\s*(?:remaining|rest|other|more)\b/gi, label: "degeneration comment (elided code)" },
  {
    re: /\/\/\s*add(?:\s+more)?\s+.*\b(?:logic|handling|implementation|checks?)\s+here\b/gi,
    label: "placeholder comment (asks to add logic later)"
  },
  {
    re: /\/\/\s*for\s+now\b.*\b(?:assume|always|return)\b/gi,
    label: "deferred-work comment (temporary behavior)"
  },
