...
function postDelegationVerification(ctx: LoopPolicyContext): CompletionBlock | null {
  ...
  return {
    ...
    message:
      ...
      "- For web projects → read_file on the main HTML file AND read_file on the key JS/code files to check for stubs, TODO comments, or placeholder logic\n" +
...
function writeWithoutVerify(ctx: LoopPolicyContext): CompletionBlock | null {
  ...
  return {
    ...
    message:
      ...
      "run_command to verify the output actually works. " +
...
function codeReviewRequired(ctx: LoopPolicyContext): CompletionBlock | null {
  ...
  return {
    ...
    message:
      "CODE REVIEW REQUIRED: You wrote code files but only ran surface checks, which " +
      "only catch load errors — they cannot verify logical correctness. " +
