...
export type PolicyScope = "sandbox" | "attachment_store" | "app_workspace" | "system"
export type PolicyDbEnvironment = "dev" | "uat" | "prod"
export type PolicyDbOperation = "query_read" | "sync_preview" | "sync_execute" | "ddl" | "dml"
...
function classifyPath(raw: string, sandboxRoot: string | null): PolicyScope {
  ...
  if (raw.startsWith("workspace://")) return "app_workspace"
  if (raw.startsWith("attachment://")) return "attachment_store"
  if (!sandboxRoot) return "system"
  try {
    const abs = isAbsolute(raw) ? resolve(raw) : resolve(sandboxRoot, raw)
    const root = resolve(sandboxRoot)
    if (abs === root || abs.startsWith(root + sep)) return "sandbox"
...
function classifyDbOperation(toolName: string, sql: string): PolicyDbOperation {
  if (toolName === "sync_preview" || toolName.endsWith("_sync_preview")) return "sync_preview"
  if (toolName === "sync_execute" || toolName.endsWith("_sync_execute")) return "sync_execute"
  if (MSSQL_DDL_RE.test(sql)) return "ddl"
  if (MSSQL_DML_RE.test(sql)) return "dml"
  if (MSSQL_READ_RE.test(sql) || toolName === "explore_mssql_schema") return "query_read"
  // Conservative fallback: treat unknown SQL as DML so it cannot bypass
  // a UAT/PROD read-only policy by failing classification.
  return sql.length > 0 ? "dml" : "query_read"
}
