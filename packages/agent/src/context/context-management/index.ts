 ...
 *
 * Tightened from 3 → 1 (Gap 2): even one stale iteration's worth of
 * raw write_file payloads dominated context. The resume-anchor /
 * full-compaction path catches anything older.
 */
const COMPACT_PRESERVE_RECENT = 1

/** Tool results shorter than this (chars) are never compacted. */
const COMPACT_MIN_SIZE = 500

/**
 * Hard cap on `write_file` / `replace_in_file` content arguments
 * preserved at the live edge (age 0). Beyond this we replace the body
 * with a placeholder regardless of recency — the actual content already
    ...
    if (m.role === MessageRole.Assistant && m.toolCalls && m.toolCalls.length > 0) {
      ...
      if (age > COMPACT_PRESERVE_RECENT) {
        ...
        needsCompaction = m.toolCalls.some((tc) => {
          ...
          // Force-trim oversized write payloads even at age 0 — the
          // bytes already landed on disk, the LLM gains nothing from
          // re-reading its own multi-KB write attempt next iteration.
          if (content.length > WRITE_ARG_MAX) return true
          const path = extractFilePath(tc.name, args)
      ...
      if (needsCompaction) {
        const compactedCalls = m.toolCalls.map((tc) => {
          ...
          const isOversizedWrite =
            (tc.name === "write_file" || tc.name === "replace_in_file") && content.length > WRITE_ARG_MAX

          if (isSuperseded || isOld || isOversizedWrite) {
