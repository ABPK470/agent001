...
export function assessDelegationDecision(
  ...
  if (econ.utilityScore < resolvedConfig.scoreThreshold) {
    return buildDecision({ shouldDelegate: false, reason: "score_below_threshold", ...baseDecision, ...econ })
  }

  // Gate 8: dependency coupling too high
  if (econ.coordinationOverhead > 0.7) {
    return buildDecision({ shouldDelegate: false, reason: "dependency_coupling_high", ...baseDecision, ...econ })
  }

  // Gate 9: negative economics (cost exceeds benefit)
  if (econ.utilityScore < 0) {
    return buildDecision({ shouldDelegate: false, reason: "negative_economics", ...baseDecision, ...econ })
  }
