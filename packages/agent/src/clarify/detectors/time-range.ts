 ...
 */
const VAGUE_TIME = /\b(recent(?:ly)?|lately|latest|current(?:ly)?|nowadays|these\s+days|this\s+period|last\s+(?:year|quarter|month|week|day)|past\s+(?:year|quarter|month|week|few\s+(?:months|weeks|days))|previous\s+(?:year|quarter|month|week|day))\b/i
...
const MONTH_YEAR = /\b(January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\b\s*\d{4}\b/i
