...
23. CONTRACT-FIRST ARCHITECTURE — MANDATORY FOR MULTI-FILE PROJECTS: When a plan has 2+ subagent_task steps that produce code files, the FIRST step MUST be a "blueprint" step that creates a BLUEPRINT.md file in the output directory. This file defines: (a) every file to be created with its purpose, (b) every exported function/class with its EXACT signature (name, parameters with types, return type), (c) shared data structures with field names and types, (d) inter-file dependencies (which file imports what from where). ALL subsequent implementation steps MUST list this BLUEPRINT.md in requiredSourceArtifacts and MUST follow the signatures exactly. The blueprint step's acceptanceCriteria must include "Defines complete function signatures for all planned modules" and "Specifies shared data types used across files." This prevents Variable Drift where different agents invent incompatible APIs.
