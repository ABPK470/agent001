...
function detectArtifactFamilies(artifacts: readonly string[]): Set<ArtifactFamily> {
...
function buildLanguageRepairGuidance(families: ReadonlySet<ArtifactFamily>): string[] {
    ...
    const batchPromises = batch.map(async (name) => {
      ...
      if (repairTask && effectiveStep.stepType === "subagent_task") {
        ...
        const blueprintRetryGuidance = docsOnlyTargets || /blueprint/i.test(sa.name)
          ...
          : ""
        const retryRules = docsOnlyTargets || /blueprint/i.test(sa.name)
          ? (hasReplaceInFile
