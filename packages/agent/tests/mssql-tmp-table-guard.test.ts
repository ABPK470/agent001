 ...
 *
 * When validateQuery is called with readOnly:true (e.g. export tool), it permits:
...
describe("validateQuery — #temp micro-ETL allowance (readOnly tool mode)", () => {
  ...
  it("blocks repeated scalar probes against the same staged temp table", () => {
    ...
    expect(validateQuery(batch, { ...RO, accessor })).toMatch(
      /aggregate the staged #temp once per business key/i
    )
  })
})
