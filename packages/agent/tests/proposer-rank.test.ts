...
import { ProposalKind, rankProposals, RiskTier, type ProposerFinding, type RankableProposal } from "@mia/sync"
...
describe("rankProposals", () => {
  ...
  it("hoists dependencies ahead of dependents", () => {
    const r = rankProposals([
      prop(
        "p-dependent",
        "consumer",
        annotation({ riskTier: RiskTier.Critical, riskScore: 90, dependsOn: ["producer"] })
      ),
  ...
  it("incorporates lineageCentrality monotonically", () => {
    ...
    const low = rankProposals(items, () => new Date("2025-01-15T12:00:00.000Z"), {
      lineageCentrality: () => 0
    })
    const high = rankProposals(items, () => new Date("2025-01-15T12:00:00.000Z"), {
      lineageCentrality: () => 1
    })
