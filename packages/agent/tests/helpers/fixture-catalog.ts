...
function col(
  ...
  dataType = "int",
  isPK = false
): { name: string; dataType: string; nullable: boolean; isPK: boolean; maxLength: number | null } {
...
export function canonicalFixtureCatalog(): CatalogGraph {
  ...
  for (let i = 0; i < 12; i++) {
    revenueBranches.push(
      mkTable("publish", `MappingRev${i}`, {
        rowCount: 5_000_000,
        columns: [
          col("pkClient", "int"),
          col("pkAccount", "int"),
          col("pkMonth", "int"),
          col("RevenueZARMTD", "decimal")
        ],
        fkOutgoing: [
          fk(`fk_rev${i}_client`, "publish", `MappingRev${i}`, "pkClient", "dim", "Client", "pkClient")
        ]
      })
    )
  ...
  for (let i = 0; i < 10; i++) {
    balanceBranches.push(
      mkTable("publish", `MappingBal${i}`, {
        rowCount: 4_000_000,
        columns: [
          col("pkAccount", "int"),
          col("pkMonth", "int"),
          col("AverageCreditBalanceZARMTD", "decimal")
        ],
        fkOutgoing: [
          fk(`fk_bal${i}_acc`, "publish", `MappingBal${i}`, "pkAccount", "dim", "Account", "pkAccount")
        ]
      })
    )
    balanceUnionParts.push(
      `SELECT pkAccount, pkMonth, AverageCreditBalanceZARMTD FROM publish.MappingBal${i}`
    )
  }
  ...
  const persistedRevenue = mkTable("persistedview", "publish.Revenue", {
    ...
    fkOutgoing: [
      fk("fk_persrev_client", "persistedview", "publish.Revenue", "pkClient", "dim", "Client", "pkClient")
    ]
  })
  ...
  dimAccount.fkIncoming.push(...balanceBranches.flatMap((t) => t.fkOutgoing), publishBalances.fkOutgoing[0]!)
