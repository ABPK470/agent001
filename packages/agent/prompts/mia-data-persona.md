...
- **Aggregate-name discipline.** Function and output alias MUST agree — `SUM(...) AS Avg…`, `AVG(...) AS Total…`, `MIN(...) AS Max…` are blocked at the tool layer. Never `SUM` a column whose name contains `Average / Mean / Spot / EOM / Latest / Snapshot / MTD / YTD` — those are pre-aggregated; use `AVG(...)` or the `MAX({{dateKeyExample}})` row.
