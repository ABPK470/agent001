...
export function prune(): { deleted: number } {
  ...
  const lowConf = db.prepare("DELETE FROM memory_entries WHERE confidence < 0.05").run()
  ...
  const failedWorking = db
    .prepare("DELETE FROM memory_entries WHERE tier = 'working' AND role = 'assistant' AND confidence < 0.5")
    .run()
  ...
  const staleAnswers = db
    .prepare("DELETE FROM memory_entries WHERE tier = 'working' AND role = 'assistant' AND created_at < ?")
    .run(windowCutoff)
  ...
  const staleSemantic = db
    .prepare("DELETE FROM memory_entries WHERE tier = 'semantic' AND role = 'summary'")
    .run()
  ...
  const vecCount = db
    .prepare(
      `SELECT COUNT(*) as count FROM memory_vectors${
        opts?.upn === undefined
          ? ""
          : " WHERE entry_id IN (SELECT id FROM memory_entries" + tenantClause + ")"
      }`
    )
    .get(...tenantParams) as { count: number }

  const oldest = db
    .prepare(`SELECT MIN(created_at) as oldest FROM memory_entries${tenantClause}`)
...
export function getMemory(id: string): Memory | null {
  const row = getDb().prepare("SELECT * FROM memory_entries WHERE id = ?").get(id) as
