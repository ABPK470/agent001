...
export function getApprovalPolicy(
  tenantId: string, targetEnv: string, tier: RiskTier,
): ApprovalPolicy {
  ...
  const defaultPolicy: ApprovalPolicyKind =
    tier === "low" ? "none" :
    tier === "medium" ? "single" : "dual"
  return {
    tenantId, targetEnv, riskTier: tier,
    policy: defaultPolicy, approvers: [], bypassRole: "admin",
  }
}

export function listApprovalPolicies(tenantId: string): ApprovalPolicy[] {
  const rows = getDb().prepare(
    `SELECT * FROM approval_policies WHERE tenant_id = ? ORDER BY target_env, risk_tier`,
  ).all(tenantId) as ApprovalPolicyRow[]
  return rows.map((row) => ({
    tenantId: row.tenant_id, targetEnv: row.target_env, riskTier: row.risk_tier,
    policy: row.policy, approvers: JSON.parse(row.approvers_json) as string[],
    bypassRole: row.bypass_role,
  }))
}
...
export function getApproval(id: string): ApprovalRow | null {
  return (getDb().prepare(`SELECT * FROM sync_approvals WHERE id = ?`).get(id) as ApprovalRow | undefined) ?? null
}
  ...
  if (row.policy === "single") {
    db.prepare(`
      UPDATE sync_approvals SET state = 'granted', granted_by_1 = ?, granted_at_1 = datetime('now')
       WHERE id = ?`).run(i.approver, i.approvalId)
...
export function rejectApproval(
  approvalId: string, rejector: string, reason: string,
): ApprovalRow {
  ...
  getDb().prepare(`
    UPDATE sync_approvals SET state = 'rejected', rejected_by = ?, rejected_at = datetime('now'), reject_reason = ?
     WHERE id = ?`).run(rejector, reason, approvalId)
  return getApproval(approvalId)!
}

export function bypassApproval(
  approvalId: string, actor: string, reason: string,
): ApprovalRow {
  const row = getApproval(approvalId)
  if (!row) throw new ApprovalError("not_found", `Approval ${approvalId} not found`)
  ...
  getDb().prepare(`
    UPDATE sync_approvals SET state = 'bypassed', bypass_by = ?, bypass_reason = ?
     WHERE id = ?`).run(actor, reason, approvalId)
  return getApproval(approvalId)!
}
...
export function consumeApprovalToken(i: ConsumeTokenInput): ConsumedToken {
  ...
  `).get(tokenHash) as {
    approval_id: string; action: "grant"|"reject"; issued_to: string;
  ...
  getDb().prepare(
    `UPDATE sync_approval_tokens SET used_at = datetime('now'), used_by = ? WHERE token_hash = ?`,
  ).run(i.by, tokenHash)
