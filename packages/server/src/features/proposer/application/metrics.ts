...
export function renderPrometheusMetrics(): string {
  ...
  push(
    ...
    rowsToLabels(
      getDb().prepare(`SELECT status, COUNT(*) AS n FROM proposer_runs GROUP BY status`).all() as Row[],
  ...
  push(
    ...
    `
        ...
        .all() as Row[],
      (r) => ({ tenant: String(r.tenant_id), risk_tier: String(r.risk_tier) })
    )
  )
  push(
    lines,
    ...
    rowsToLabels(
      getDb().prepare(`SELECT status, COUNT(*) AS n FROM sync_proposals GROUP BY status`).all() as Row[],
      (r) => ({ status: String(r.status) })
    )
  )
  push(
    ...
    rowsToLabels(
      getDb().prepare(`SELECT state, COUNT(*) AS n FROM sync_approvals GROUP BY state`).all() as Row[],
  ...
  push(
    ...
    rowsToLabels(
      getDb()
        ...
        .all() as Row[],
      (r) => ({ status: String(r.status), channel: String(r.channel) })
    )
  )
  return lines.join("\n") + "\n"
}
