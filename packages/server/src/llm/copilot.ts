...
export class CopilotClient implements LLMClient {
  ...
  private async chatStream(messages: Message[], tools: Tool[], opts: { signal?: AbortSignal; maxTokens?: number; onToken?: (token: string) => void }): Promise<LLMResponse> {
    ...
    if (finishReason === "length") {
      throw new Error("LLM response truncated (finish_reason=length). The model hit its completion token limit before finishing.")
    }

    return {
      content: content || null,
      toolCalls: [...toolCallMap.values()].map((tc): ToolCall => ({
        id: tc.id,
        name: tc.name,
