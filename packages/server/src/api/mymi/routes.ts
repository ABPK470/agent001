...
export function registerMymiRoutes(app: FastifyInstance, host: AgentHost): void {
  ...
  app.get<QS>("/api/mymi/overview", async (req) => {
    ...
    const bySchema = new Map<
      string,
      { tableCount: number; viewCount: number; totalRows: number; totalMb: number }
    >()
    for (const [, table] of catalog.tables) {
      if (!bySchema.has(table.schema))
        bySchema.set(table.schema, { tableCount: 0, viewCount: 0, totalRows: 0, totalMb: 0 })
  ...
  app.get<{ Querystring: { db?: string; q?: string; schemas?: string } }>(
    ...
    async (req, reply) => {
      ...
      results.sort((a, b) =>
        a.matchKind !== b.matchKind ? (a.matchKind === "object" ? -1 : 1) : b.rowCount - a.rowCount
      )
  ...
  app.get<QS & { Params: { schema: string } }>("/api/mymi/schema/:schema", async (req, reply) => {
    ...
    for (const [, table] of catalog.tables) {
      ...
      results.push({
        name: table.name,
        type: table.type === "TABLE" ? ("table" as const) : ("view" as const),
        rowCount: table.rowCount ?? catalog.viewSourceRows.get(table.qualifiedName) ?? 0,
        sizeMb: 0,
  ...
  app.get<QS & { Params: { schema: string; table: string } }>(
    ...
    async (req, reply) => {
      ...
      for (const fk of entry.fkOutgoing)
        fkByCol.set(fk.fromColumn, {
          fkSchema: fk.toSchema,
          fkTable: fk.toTable,
      ...
      return {
        ...
        implicit: catalog.getImplicitJoins(entry.qualifiedName, 30).map((edge) => ({
          ...
          tables: (edge.tables ?? [])
            ...
            .map((qn) => ({
              qualifiedName: qn,
              rowCount: catalog.tables.get(qn)?.rowCount ?? null
            }))
        }))
      }
    }
  ...
  app.get<{ Params: { schema: string; table: string }; Querystring: { db?: string; limit?: string } }>(
    ...
    async (req, reply) => {
      ...
      const columns = colMeta
        ? Object.keys(colMeta).map((col) => ({
            name: col,
            type: colMeta[col]?.type?.declaration ?? "unknown"
          }))
  ...
  app.get<QS>("/api/mymi/datamodel", async (req) => {
    ...
    for (const [, table] of catalog.tables) {
      objects.push({
        schema: table.schema,
        name: table.name,
        isTable: table.type === "TABLE",
        rowCount: table.rowCount ?? catalog.viewSourceRows.get(table.qualifiedName) ?? 0,
        sizeMb: 0,
        columnCount: table.columns.length,
        fkOut: table.fkOutgoing.length,
        fkIn: table.fkIncoming.length
      })
    }
    ...
    for (const [, edges] of catalog.adjacency) {
      for (const edge of edges) {
        ...
        relSet.add(k)
        relations.push({
          srcSchema: edge.fk.fromSchema,
          srcTable: edge.fk.fromTable,
          refSchema: edge.fk.toSchema,
          refTable: edge.fk.toTable
        })
      }
    }
