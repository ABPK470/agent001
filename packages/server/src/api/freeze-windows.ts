...
export function registerFreezeWindowRoutes(app: FastifyInstance): void {
	...
	app.post<{ Body: { id: string; displayName: string; description: string; startsAt: string; endsAt: string } }>("/api/sync/freeze-windows", async (req, reply) => {
		if (!req.session?.isAdmin) { reply.code(403); return { error: "admin only" } }
		const body = req.body
		if (!body?.id) { reply.code(400); return { error: "missing id" } }
		const tenantId = resolveTenant(req)
		try {
