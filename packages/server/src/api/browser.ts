...
import { createCredential, deleteCredential, listCredentials, type CredentialKind } from "../adapters/browser/credentials.js"
import { awaitHandoff, completeHandoff, getHandoff, listHandoffs, mintHandoff, revokeHandoff } from "../adapters/browser/handoff.js"
...
export function registerBrowserRoutes(app: FastifyInstance): void {
	...
	app.post<{ Body: { label?: string; kind?: CredentialKind; target_origin?: string; payload?: unknown } }>("/api/browser/credentials", async (req, reply) => {
		const upn = requireUpn(req, reply)
		if (!upn) return { error: "auth required" }
	...
	app.post<{ Body: { browser_session_id?: string; reason?: "captcha" | "2fa" | "manual"; ttl_ms?: number } }>("/api/browser/handoff", async (req, reply) => {
		const upn = requireUpn(req, reply)
		if (!upn) return { error: "auth required" }
		...
		const rec = mintHandoff({ ownerUpn: upn, browserSessionId: browser_session_id, reason: reason ?? "manual", ...(ttl_ms ? { ttlMs: ttl_ms } : {}) })
		return { handoff: rec }
	})
	...
	app.post<{ Body: { pattern?: string; effect?: "allow" | "deny"; reason?: string; global?: boolean } }>("/api/browser/policy", async (req, reply) => {
		const upn = requireUpn(req, reply)
		if (!upn) return { error: "auth required" }
		...
		const rule = addPolicyRule({ ownerUpn: global ? null : upn, pattern, effect, ...(reason ? { reason } : {}) })
		return { rule }
	})
