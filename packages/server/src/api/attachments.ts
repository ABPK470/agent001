...
export function registerAttachmentRoutes(app: FastifyInstance): void {
	app.post<{ Body: UploadBody }>("/api/attachments", { bodyLimit: MAX_UPLOAD_BYTES + 64 * 1024 }, async (req, reply) => {
	...
	app.get<{ Querystring: { scope?: AttachmentScope; runId?: string; q?: string } }>("/api/attachments", async (req, reply) => {
		if (!requireSession(req, reply)) return
