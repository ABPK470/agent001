...
export function renderPriorTurnsBlock(turns: readonly PriorTurn[]): string {
  ...
  lines.push(
    ...
    "",
    "SCOPE: Prior goals are history only. Do NOT continue or extend a prior",
    "turn's task unless the CURRENT goal explicitly asks for it (pronouns,",
    '\u201calso\u201d, \u201cnow sync that\u201d, \u201csame as before\u201d, etc.). Tooling for this run',
    "reflects the current goal's classified intent \u2014 do not adopt a different",
    "workflow because memory or an earlier turn used other tools.",
    "</prior_turns>"
  )
  return lines.join("\n")
}
