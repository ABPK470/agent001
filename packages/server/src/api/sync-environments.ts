...
import { ENV_ACCESS_MODES, ENV_ROLES, isEnvAccessMode, isEnvRole, type EnvOperation, type SyncEnvironment } from "@mia/sync"
...
function sanitise(body: Record<string, unknown>): Partial<Editable> | string {
	...
	if (body["displayName"] !== undefined) {
		if (typeof body["displayName"] !== "string" || body["displayName"].trim() === "") return "displayName must be a non-empty string"
		out.displayName = body["displayName"].trim()
	}
	if (body["color"] !== undefined) {
		if (typeof body["color"] !== "string" || body["color"].trim() === "") return "color must be a non-empty string"
	...
	if (body["ringOrder"] !== undefined) {
		if (typeof body["ringOrder"] !== "number" || !Number.isFinite(body["ringOrder"])) return "ringOrder must be a number"
...
function serialiseEnvironment(env: SyncEnvironment, actor: string | null, createdAt?: string): db.DbSyncEnvironment {
...
function parseEnvironmentRow(row: db.DbSyncEnvironment): SyncEnvironment & { updatedAt: string; updatedBy: string | null } {
...
export function registerSyncEnvironmentRoutes(app: FastifyInstance, host: AgentHost): void {
	...
	app.post<{ Body: { name?: string } & Record<string, unknown> }>("/api/sync-environments", async (req, reply) => {
		if (!req.session?.isAdmin) { reply.code(403); return { error: "admin only" } }
		const name = typeof req.body?.name === "string" ? req.body.name.trim() : ""
		if (!name) { reply.code(400); return { error: "name is required" } }
		if (!hasConnection(host, name)) { reply.code(400); return { error: `unknown MSSQL connection "${name}"` } }
		if (db.getSyncEnvironment(name)) { reply.code(409); return { error: `environment already exists: ${name}` } }
		...
		broadcast({ type: EventType.SyncEnvUpdate, data: { name, action: "create", actor: req.session.upn } })
		return { ok: true }
	})
	...
	app.put<{ Params: { name: string }; Body: Record<string, unknown> }>("/api/sync-environments/:name", async (req, reply) => {
		...
		if (!row) { reply.code(404); return { error: `unknown env "${req.params.name}"` } }
		const sanitised = sanitise(req.body ?? {})
		if (typeof sanitised === "string") { reply.code(400); return { error: sanitised } }
		const next = { ...(JSON.parse(row.body_json) as SyncEnvironment), ...sanitised, name: req.params.name }
		...
		broadcast({ type: EventType.SyncEnvUpdate, data: { name: req.params.name, action: "update", actor: req.session.upn } })
		return { ok: true }
	})
	...
	app.delete<{ Params: { name: string } }>("/api/sync-environments/:name", async (req, reply) => {
		...
		broadcast({ type: EventType.SyncEnvUpdate, data: { name: req.params.name, action: "delete", actor: req.session.upn } })
		return { ok: true }
	})
}
