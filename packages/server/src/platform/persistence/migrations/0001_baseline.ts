...
const BASELINE_SQL = `
    -- ── llm_config (singleton) ───────────────────────────────────
    CREATE TABLE IF NOT EXISTS llm_config (
      id INTEGER PRIMARY KEY CHECK (id = 1),
      provider TEXT NOT NULL DEFAULT 'databricks',
      model TEXT NOT NULL DEFAULT 'databricks-gpt-5-4',
    ...
    CREATE INDEX IF NOT EXISTS idx_runs_thread ON runs(thread_id, created_at ASC);
    ...
    CREATE TABLE IF NOT EXISTS conversations (
      id            TEXT PRIMARY KEY,
      channel_type  TEXT NOT NULL CHECK (channel_type IN ('teams')),
    ...
    CREATE INDEX IF NOT EXISTS idx_conv_channel_sender ON conversations(channel_type, sender_id);
    CREATE INDEX IF NOT EXISTS idx_conv_run ON conversations(active_run_id);
    ...
    CREATE INDEX IF NOT EXISTS idx_outbound_conv ON outbound_messages(conversation_id);
    ...
    CREATE INDEX IF NOT EXISTS idx_delivery_msg ON delivery_attempts(message_id);

    CREATE TABLE IF NOT EXISTS channel_configs (
      type         TEXT PRIMARY KEY CHECK (type IN ('teams')),
