...
export function getAttachment(id: string): AttachmentRow | undefined {
  return db().prepare("SELECT * FROM attachments WHERE id = ? AND status != 'deleted'").get(id) as AttachmentRow | undefined
}
...
export function listAttachments(filter: ListAttachmentsFilter = {}): AttachmentRow[] {
  ...
  if (filter.ownerUpn)  { where.push("owner_upn = @ownerUpn");   params["ownerUpn"]  = filter.ownerUpn }
  if (filter.q) {
...
export function setAttachmentProcessed(id: string, textExtractUri: string | null): void {
  db().prepare(`
    UPDATE attachments
...
export function listAttachmentTags(attachmentId: string): AttachmentTagRow[] {
  return db().prepare("SELECT * FROM attachment_tags WHERE attachment_id = ?").all(attachmentId) as AttachmentTagRow[]
}
...
export function listAttachmentImports(runId: string): AttachmentImportRow[] {
  return db().prepare("SELECT * FROM attachment_imports WHERE run_id = ? ORDER BY imported_at DESC").all(runId) as AttachmentImportRow[]
}
