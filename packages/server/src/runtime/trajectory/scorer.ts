  ...
  for (const { seq, event, timestamp } of t.events) {
    ...
    switch (event.kind) {
      case TrajectoryEventKind.Goal: {
        const goalEvent = event as Extract<typeof event, { kind: typeof TrajectoryEventKind.Goal }>
        lines.push(`  [${seq}] ${time} GOAL: ${goalEvent.text.slice(0, 80)}`)
        break
      }
      case TrajectoryEventKind.Thinking: {
        const thinkingEvent = event as Extract<typeof event, { kind: typeof TrajectoryEventKind.Thinking }>
        lines.push(`  [${seq}] ${time} THINK: ${thinkingEvent.text.slice(0, 60)}…`)
        break
      }
      case TrajectoryEventKind.ToolCall: {
        const toolCallEvent = event as Extract<typeof event, { kind: typeof TrajectoryEventKind.ToolCall }>
        lines.push(`  [${seq}] ${time} CALL: ${toolCallEvent.tool}(${toolCallEvent.argsSummary})`)
        break
      }
      case TrajectoryEventKind.ToolResult: {
        ...
        >
        lines.push(`  [${seq}] ${time} RESULT: ${toolResultEvent.text.slice(0, 60)}…`)
        break
      }
      case TrajectoryEventKind.ToolError: {
        const toolErrorEvent = event as Extract<typeof event, { kind: typeof TrajectoryEventKind.ToolError }>
        lines.push(`  [${seq}] ${time} ERROR: ${toolErrorEvent.text.slice(0, 60)}…`)
        break
      }
      case TrajectoryEventKind.Iteration: {
        const iterationEvent = event as Extract<typeof event, { kind: typeof TrajectoryEventKind.Iteration }>
        lines.push(`  [${seq}] ${time} ITER: ${iterationEvent.current}/${iterationEvent.max}`)
        break
      }
      case TrajectoryEventKind.DelegationStart: {
        const delegationStartEvent = event as Extract<
          typeof event,
          { kind: typeof TrajectoryEventKind.DelegationStart }
        >
        lines.push(`  [${seq}] ${time} DELEGATE: ${delegationStartEvent.childGoal.slice(0, 60)}`)
        break
      }
      case TrajectoryEventKind.DelegationEnd: {
        const delegationEndEvent = event as Extract<
          typeof event,
          { kind: typeof TrajectoryEventKind.DelegationEnd }
        >
        lines.push(`  [${seq}] ${time} DELEGATE-DONE: ${delegationEndEvent.result.slice(0, 60)}`)
        break
      }
      case TrajectoryEventKind.Answer: {
        const answerEvent = event as Extract<typeof event, { kind: typeof TrajectoryEventKind.Answer }>
        lines.push(`  [${seq}] ${time} ANSWER: ${answerEvent.text.slice(0, 80)}`)
        break
      }
      case TrajectoryEventKind.Error: {
        const errorEvent = event as Extract<typeof event, { kind: typeof TrajectoryEventKind.Error }>
        lines.push(`  [${seq}] ${time} FATAL: ${errorEvent.text.slice(0, 80)}`)
        break
      }
    }
  }
