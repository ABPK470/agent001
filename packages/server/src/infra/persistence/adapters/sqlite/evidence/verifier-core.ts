...
export async function verifyEvidence(i: VerifyInput): Promise<VerificationReport> {
  ...
  if (chain.length > 0) {
    return {
      code: VerificationCode.HashChain,
      ok: false,
      message: `hash chain mismatch for: ${chain.join(", ")}`,
      failed: chain,
      signerId: env.signature.signerId,
      contentHash: env.signature.contentHash
    }
  }
  if (!i.signer) {
    return {
      code: VerificationCode.Io,
      ok: false,
  ...
  let valid = false
  try {
    valid = await i.signer.verify(envelopeBodyBytes(env), env.signature.value)
  } catch (e) {
    return {
      code: VerificationCode.Io,
      ok: false,
  ...
  if (!valid) {
    return {
      code: VerificationCode.Signature,
      ok: false,
