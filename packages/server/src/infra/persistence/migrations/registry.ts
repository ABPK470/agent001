...
`,
  ...
  },
  {
    ...
    up: {
      // Playwright/browser stack removed — kept as ledger no-op; drops in v11.
      mssql: async () => {},
      postgres: async (executor) => {
    ...
    up: {
      mssql: async (executor) => {
        await mssqlExec(
          executor,
          `
IF OBJECT_ID(N'dbo.browser_audit_log', N'U') IS NOT NULL DROP TABLE dbo.browser_audit_log;
IF OBJECT_ID(N'dbo.browser_domain_policy_configs', N'U') IS NOT NULL DROP TABLE dbo.browser_domain_policy_configs;
IF OBJECT_ID(N'dbo.browser_proxy_config', N'U') IS NOT NULL DROP TABLE dbo.browser_proxy_config;
IF OBJECT_ID(N'dbo.browser_credentials', N'U') IS NOT NULL DROP TABLE dbo.browser_credentials;
IF OBJECT_ID(N'dbo.browser_contexts', N'U') IS NOT NULL DROP TABLE dbo.browser_contexts;
`,
        )
      },
      postgres: async (executor) => {
