...
| 4 | Second dialect (**hosted default: mssql**) + multi-dialect migrator | **In progress** — single Kysely handle; registry v1–6; `platformNow` in core repos; boot still refuses |
...
| `migrations/registry.ts` | Multi-dialect peer DDL (mssql v1–6) |
| `sql-time.ts` | Dialect-aware `platformNow()` / `coalescePlatformNow()` |
