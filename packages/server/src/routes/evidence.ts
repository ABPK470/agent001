/**
 * F1.8 — Evidence routes.
 *
 *   GET  /api/evidence                       list recent evidence rows for tenant
 *   GET  /api/evidence/by-plan/:planId       latest envelope row for a plan
 *   GET  /api/evidence/:id/envelope.json     download signed envelope
 *   GET  /api/evidence/:id/evidence.pdf      download human-readable PDF
 *   POST /api/evidence/:id/verify            run server-side verification
 *
 * Storage root is provided once at registration (mirrors how
 * `registerSyncRoutes` takes `projectRoot`). The signer comes from
 * `EVIDENCE_SIGNER_KIND` env at boot.
 */
