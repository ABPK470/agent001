...
async function setup() {
  ...
  _setDb(testDb)
  _migrate(testDb)
  return import("../src/db/proposals.js")
}
...
describe("proposals storage (F1.2)", () => {
  it("creates a proposer run + ingests findings + dedupes by fingerprint", async () => {
    const m = await setup()
    const runId = m.createProposerRun({ tenantId: "_default", source: "uat", target: "prod", triggeredBy: "u", trigger: "manual" })
    const a = m.ingestFindings("_default", runId, [finding(), finding({ entityId: "c2", fingerprint: "fp-c2" })])
    expect(a).toHaveLength(2)
    // re-ingest same fingerprints — should dedupe
    const b = m.ingestFindings("_default", runId, [finding(), finding({ entityId: "c2", fingerprint: "fp-c2" })])
  ...
  it("transitions status with append-only history", async () => {
    ...
    const updated = m.updateProposalStatus({ id: id!, to: ProposalStatus.AwaitingApproval, actor: "alice" })
  ...
  it("persists annotation + rank score", async () => {
    ...
    const [id] = m.ingestFindings("_default", runId, [finding()])
    m.saveAnnotation(id!, {
      riskTier: RiskTier.High, riskScore: 70,
      rationale: "X. Y. Z.", recommendedWindow: "any",
      dependsOn: [], warnings: [],
    }, false)
  ...
  it("finishes a proposer run with totals", async () => {
    const m = await setup()
    const id = m.createProposerRun({ tenantId: "_default", source: "uat", target: "prod", triggeredBy: "u", trigger: "schedule" })
    m.markProposerRunRunning(id)
    m.finishProposerRun({ id, status: "completed", counts: { scanned: 10, produced: 3, errors: 0 }, durationMs: 1234, error: null })
