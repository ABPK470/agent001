...
describe("memory tenancy \u2014 cross-tier UPN isolation", () => {
  ...
  it("episodic summaries are tenant-scoped", async () => {
    ...
    const aliceHits = await mem.searchEntries("alphaonlytokenxyz", {
      tier: "episodic",
      budget: { maxTokens: 4000, maxItems: 5 },
      upn: "alice@corp"
    })
    const bobOnAlice = await mem.searchEntries("alphaonlytokenxyz", {
      tier: "episodic",
      budget: { maxTokens: 4000, maxItems: 5 },
      upn: "bob@corp"
    })
    const bobHits = await mem.searchEntries("bravoonlytokenxyz", {
      tier: "episodic",
      budget: { maxTokens: 4000, maxItems: 5 },
      upn: "bob@corp"
    })
  ...
  it("memory_vectors mirrors upn/shared and SQL filter prevents cross-tenant rows", async () => {
    const mem = await setupMemory()
    const { getDb } = await import("../src/infra/persistence/adapters/sqlite/index.js")
