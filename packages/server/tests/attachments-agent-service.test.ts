...
async function withAttachmentService<T>(
  ...
  const { createServerAttachmentService } = await import("../src/infra/persistence/attachments/index.js")
...
describe("server attachment service", () => {
  ...
  it("throws a clear error when called outside a run context", async () => {
    const { _setDb, _migrate } = await import("../src/infra/persistence/db/index.js")
    const { serverAttachmentService } = await import("../src/infra/persistence/attachments/index.js")
    _setDb(testDb)
    _migrate(testDb)
    seedTestUsers(testDb)
