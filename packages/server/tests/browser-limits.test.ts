...
beforeEach(async () => {
  const { _resetLimits } = await import("../src/browser/limits.js")
  _resetLimits()
})
...
describe("browser limits — concurrency", () => {
  it("acquires up to MAX_PER_USER then queues", async () => {
    const { acquireUserSlot, _userInFlight, limitsConfig } = await import("../src/browser/limits.js")
...
describe("browser limits — token bucket", () => {
  it("starts full and consumes one token per request", async () => {
    const { tryConsumeDomainToken, limitsConfig } = await import("../src/browser/limits.js")
  ...
  it("isolates tenants and domains", async () => {
    const { tryConsumeDomainToken, _resetLimits, limitsConfig } = await import("../src/browser/limits.js")
    _resetLimits()
