...
describe("tool-cache", () => {
  it("memoises deterministic compute() across calls within a session", async () => {
    ...
    const { getOrCompute } = await import("../src/infra/persistence/tool-cache.js")
    let calls = 0
    const compute = async () => {
      calls++
      return { hello: "world" }
  ...
  it("expired entries are treated as misses and removable by cleanup", async () => {
    const { getOrCompute, readCache, cleanupExpiredCache, getCacheStats } =
      await import("../src/infra/persistence/tool-cache.js")
    let calls = 0
    const compute = async () => {
      calls++
      return "v"
  ...
  it("rejects unsafe upns so cache cannot escape its partition", async () => {
    const { writeCache, readCache } = await import("../src/infra/persistence/tool-cache.js")
