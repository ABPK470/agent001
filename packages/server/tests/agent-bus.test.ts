...
describe("AgentBus persistence (B.1)", () => {
  it("writes every publish to the agent_messages table", async () => {
    ...
    bus.publish({
      ...
      fromAgent: "Parent",
      content: "done",
      protocol: BusProtocol.Result
    })

    const rows = testDb
      .prepare(
        `SELECT topic, protocol, content FROM agent_messages WHERE root_run_id = ? ORDER BY created_at ASC`
      )
      .all(rootRunId) as Array<{ topic: string; protocol: string; content: string }>
...
describe("wait_for_response (B.2)", () => {
  it("resolves when an Answer arrives after subscription", async () => {
    ...
    const sendResult = (await sendMessage.execute({
      topic: "q1",
      content: "what is x?",
  ...
  it("fast-paths a pre-existing Answer in the DB", async () => {
    ...
    const askerTools = createBusTools(bus, "asker", "Asker")
    const text = (await askerTools
      .find((t) => t.name === "wait_for_response")!
      .execute({ message_id: question.id, timeout_ms: 5_000 })) as string
...
describe("Per-child bus identity (B.3)", () => {
  it("createBusTools binds runId+agentName per call so children publish as themselves", async () => {
    ...
    const rows = testDb
      .prepare(
        `SELECT from_run_id, from_agent, content FROM agent_messages WHERE root_run_id = ? ORDER BY created_at ASC`
      )
