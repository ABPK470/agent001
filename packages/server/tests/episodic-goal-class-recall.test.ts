...
} from "../src/infra/persistence/adapters/sqlite/memory/episodic-choreography.js"
import {
  augmentGoalQueryForFts,
  episodicShortcutMatchesGoal,
  extractGoalClasses,
  goalClassesShareAffinity
} from "../src/infra/persistence/adapters/sqlite/memory/goal-class.js"
...
describe("extractGoalClasses", () => {
  ...
  it("tags reconciliation-shaped goals separately from aggregate counts", () => {
    ...
    const countClasses = extractGoalClasses("how many distinct pipelines are in DE and in UAT")
    expect(countClasses).toContain("aggregateby")
    expect(countClasses).not.toContain("syncreconcile")
  ...
  it("blocks shortcut across incompatible task shapes", () => {
    expect(
      episodicShortcutMatchesGoal("how many distinct pipelines are in DE and in UAT", [
        {
          content: "Goal: reconcile pipelineActivity uat vs dev",
          metadata: { goalClasses: ["syncreconcile"] }
        }
      ])
    ).toBe(false)
  })
  ...
  it("blocks shortcut when only ambient comparison overlaps reconcile memory", () => {
    expect(
      episodicShortcutMatchesGoal("revenue yoy growth trend", [
        {
          content: "Goal: reconcile pipelineActivity uat vs dev",
          metadata: { goalClasses: ["syncreconcile", "comparison"] }
        }
      ])
    ).toBe(false)
  })
