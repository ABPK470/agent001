...
const fakeVerdictTool = (execute: (args: Record<string, unknown>) => Promise<string>): Tool => ({
...
describe("runReflectionTurn", () => {
  ...
  it("rejects non-allowed tool names", async () => {
    ...
    const llm = fakeLlm({
      content: null,
      toolCalls: [{ id: "tc1", name: "query_mssql", arguments: { sql: "DROP TABLE X" } }]
    })
    const r = await runReflectionTurn({
      runId: "r1",
      goal: "g",
  ...
  it("does not throw when verdict tool execute() rejects", async () => {
    const tool = fakeVerdictTool(async () => {
